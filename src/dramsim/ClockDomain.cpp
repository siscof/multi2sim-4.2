...
namespace ClockDomain
{
	...
	ClockDomainCrosser::ClockDomainCrosser(ClockUpdateCB const &callback)
		: callback(callback.clone()), clock1(1UL), clock2(1UL), counter1(0UL), counter2(0UL)
	{
	}


	ClockDomainCrosser::ClockDomainCrosser(uint64_t _clock1, uint64_t _clock2, ClockUpdateCB const &callback)
		: callback(callback.clone()), clock1(_clock1), clock2(_clock2), counter1(0), counter2(0)
	{
		//cout << "CTOR: callback address: " << (uint64_t)(this->callback) << "\t ratio="<<clock1<<"/"<<clock2<< endl;
	}


	ClockDomainCrosser::ClockDomainCrosser(double ratio, ClockUpdateCB const &callback)
		: callback(callback.clone()), counter1(0), counter2(0)
	{
	...
	ClockDomainCrosser::~ClockDomainCrosser()
	{
		delete callback;
	}
	...
	int TestObj::test()
	{
		auto callback = DRAMSim::Callback<TestObj, void>(this, &TestObj::cb);
