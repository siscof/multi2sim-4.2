	...
	int active_fb)
{
	...
	for (i = 0; i < si_gpu_max_wavefronts_per_wavefront_pool; i++)
	{
		...
		if (wavefront->wavefront_pool_entry->wait_for_mem)
		{
			...
			if(wavefront->wavefront_pool_entry->waiting_vm_cnt >= wavefront->wavefront_pool_entry->vm_cnt
				&& wavefront->wavefront_pool_entry->waiting_exp_cnt >= wavefront->wavefront_pool_entry->exp_cnt
				&& wavefront->wavefront_pool_entry->waiting_lgkm_cnt >= wavefront->wavefront_pool_entry->lgkm_cnt)
			{
				si_wavefront_add_stall(wavefront);
				...
				wavefront->wavefront_pool_entry->waiting_vm_cnt = 0;
				wavefront->wavefront_pool_entry->waiting_exp_cnt = 0;
				wavefront->wavefront_pool_entry->waiting_lgkm_cnt = 0;
			}
			else
			{
