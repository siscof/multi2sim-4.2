...
void si_compute_unit_config_vmb(struct si_compute_unit_t *compute_unit)
{
        ...
        for (int i = 0; i < compute_unit->num_wavefront_pools; i++)
	{
            ...
            {
                compute_unit->wavefront_pools[i]->entries[j]->mem_buffer = list;
            }
        }
        return;
    }
	...
	int active_fb)
{
	...
	for (i = 0; i < si_gpu_max_wavefronts_per_wavefront_pool; i++)
	{
		...
		if (wavefront->wavefront_pool_entry->wavefront_finished)
		{
			...
			{
                            if(wavefront->wavefront_pool_entry->wait_for_mem_cycle == 0)
                                    wavefront->wavefront_pool_entry->wait_for_mem_cycle = asTiming(si_gpu)->cycle;
                        ...
                        if(wavefront->wavefront_pool_entry->wait_for_mem_cycle != 0)
                        {
                            assert(wavefront->wavefront_pool_entry->wait_for_mem == 0);
