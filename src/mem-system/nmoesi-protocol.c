...
//			new_stack = mod_stack_create(stack->id, mod_get_low_mod(mod, stack->addr), stack->addr,	EV_MOD_NMOESI_LOAD_FINISH, stack);
			...
			new_stack = mod_stack_create(stack->id, mod_get_low_mod(mod, stack->addr), stack->addr, EV_MOD_NMOESI_LOAD_SEND, stack);
        	esim_schedule_event(EV_MOD_NMOESI_LOAD_SEND, new_stack, 0);	
	...
	/*}else{
		mshr_enqueue(mod->mshr, stack, EV_MOD_NMOESI_LOAD_LOCK);
	}*/	
	return;
	}
...
if (event == EV_MOD_NMOESI_LOAD_ACTION)
{
	...
	if (stack->state)
	{
		...
		//(mshr_lock(mod->mshr,stack))
		//
			esim_schedule_event(EV_MOD_NMOESI_READ_REQUEST, new_stack, 0);
		/*lse{
			mshr_enqueue(mod->mshr, stack, EV_MOD_NMOESI_READ_REQUEST);
		}	*/
	...
	if (event == EV_MOD_NMOESI_LOAD_MISS)
	{
		...
		if (stack->err)
		{
			...
			dir_entry_unlock_stack(mod->dir, stack->set, stack->way,stack);
			if(stack->mshr_locked)
			{
				mshr_unlock2(mod);
				stack->mshr_locked = 0;
			...
			cc_launch_next_transaction(mod->coherence_controller,stack);
	...
	{
		//send message
		/*if(0 == 1 && stack->ret_stack != NULL && SALTAR_L1 == 1 && mod->level == 2)
		{
			stack->request_dir = mod_request_down_up;
			stack->ret_stack->request_dir = mod_request_down_up;
			esim_schedule_event(EV_MOD_NMOESI_LOAD_SEND, stack, 0);
			return;
        }*/
...
void mod_handler_nmoesi_nc_store(int event, void *data)
{
 ...
 if(event == EV_MOD_NMOESI_NC_STORE_SEND)
	...
	if (event == EV_MOD_NMOESI_NC_STORE_WRITEBACK)
	{
		...
		{
			printf("se ha detectado un bloque modificado o owned en una escritura no-coherente\n");
			stack->eviction = 1;
			...
			/*if(mshr_lock(mod->mshr, stack))
			{
				esim_schedule_event(EV_MOD_NMOESI_EVICT, stack, 0);
			}else{
				mshr_enqueue(mod->mshr, stack, EV_MOD_NMOESI_EVICT);
			}*/
...
void mod_handler_nmoesi_find_and_lock(int event, void *data)
{
	...
	if (event == EV_MOD_NMOESI_FIND_AND_LOCK_FINISH)
	{
		...
		if (stack->err)
		{
			...
			cc_launch_next_transaction(mod->coherence_controller,stack->ret_stack);
...
void mod_handler_nmoesi_evict(int event, void *data)
{
	...
	if (event == EV_MOD_NMOESI_EVICT_PROCESS)
	{
		...
		dir_entry_unlock_stack(target_mod->dir, stack->set, stack->way,stack);
		if(stack->mshr_locked)
		{
			mshr_unlock2(target_mod);
			stack->mshr_locked = 0;
		...
		cc_launch_next_transaction(target_mod->coherence_controller,stack);
	...
	if (event == EV_MOD_NMOESI_READ_REQUEST_REPLY)
	{
		...
		if (stack->request_dir == mod_request_up_down)
		{
			...
			//	if(!stack->coalesced)
				...
				mod_access_finish(target_mod, stack);
		         //       if((stack->ret_event == EV_MOD_NMOESI_LOAD_MISS) && stack->coalesced && stack->state)
                	//	        add_CoalesceHit(target_mod->level);
...
void mod_handler_nmoesi_peer(int event, void *data)
{
	...
	printf("WARNING: EV_MOD_NMOESI_PEER_SEND GENERADO");	
